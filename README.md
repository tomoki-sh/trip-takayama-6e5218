...
## 手順 ④: 内容を更新したいとき ―  `publish.sh` を使う

**これが唯一の公開手順です。** 手で `git add / commit / push` しないでください。

```bash
./tools/publish.sh "スポットを追加"
```

このコマンドが順に次をやります。

1. **`app.js` + `style.css` の内容ハッシュを `index.html` の `?v=` に埋める**（後述）
2. 構文チェックと**検証4本**（`test_site` / `test_cardtools` / `test_info` / `test_days`）
   → **1つでも落ちたら push しません**
3. commit → push
4. **公開版がローカルと一致するまで待って確認**し、公開URLを表示

`DRY_RUN=1 ./tools/publish.sh` で、`?v=` の更新と検証だけを試せます（push しません）。

### なぜ `?v=` が要るのか

GitHub Pages は `index.html` / `app.js` / `style.css` を**それぞれ独立に10分キャッシュ**します
（`cache-control: max-age=600`）。しかもブラウザは再読み込みのとき、
**HTMLは取り直すのにJS/CSSはキャッシュのまま使う**ことがあります。その結果、

- 新しい `index.html`（タブは増えている）
- 古い `app.js`（そのタブを描く関数が無い）

という**ちぐはぐな組み合わせ**が成立し、**タブはあるのに中身が空**になります。
実際に「ワンちゃん特集タブが出るのに中身が表示されない」という形で発生しました。

`?v=` の値はファイルの内容から計算するので、**中身が変われば自動で変わり、変わらなければそのまま**です。
**人間が番号を上げる運用にはしていません。** 忘れたときに何の警告も出ないまま
同じ不具合が再発する＝安全装置として弱いからです。

なお `index.html` 自体のキャッシュは残るため、**push 直後の最大10分は古い内容が見えること**があります。
これは「反映が遅い」だけで、壊れて見える状態にはなりません。急ぐときはスーパーリロード
（iPhone Safari は「設定 → Safari → 履歴とWebサイトデータを消去」、PCは Cmd+Shift+R）。
