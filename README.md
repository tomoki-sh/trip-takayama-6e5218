...
## ⚠️ 更新するときは index.html の `?v=` を +1 する

GitHub Pages は `index.html` / `app.js` / `style.css` を**それぞれ独立に10分キャッシュ**する
（`cache-control: max-age=600`）。そのため何もしないと、閲覧者の端末で

- 新しい `index.html`（タブは増えている）
- 古い `app.js`（新しいタブを描く関数が無い）

という**ちぐはぐな組み合わせ**が起きて、**タブはあるのに中身が空**になる。
実際に「ワンちゃん特集タブが出るのに中身が表示されない」という形で発生した。

`index.html` の2か所に `?v=` を付けてあるので、**内容を更新して push するときは必ず数字を +1** する:
...
数字を変えるとURLが変わるので、ブラウザは必ず新しいファイルを取りに行く。
`index.html` 自体も最大10分キャッシュされるため、**push 直後の10分間は古いままのことがある**
（それ以降に開いた人は確実に最新になる）。急ぐときはスーパーリロード
